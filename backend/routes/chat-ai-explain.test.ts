// Route tests for POST /api/chat/explain.
// Mirrors chat-ai-translate.test.ts: minimal Express app with stubbed
// auth/session and a vi.mock'd OpenAI client so success-path requests
// do not call the real upstream.
          ...
          choices: [
            {
              message: {
                content: JSON.stringify({
                  simpleMeaning: "The speaker is greeting you.",
                  keyVocabulary: [
...
const explainModule = await import("./chat-ai-explain");
const explainRouter = explainModule.default;
const { __testing } = explainModule;
...
beforeAll(async () => {
  ...
  authedServer = http.createServer(buildApp("test-user-explain"));
...
describe("POST /chat/explain", () => {
  it("returns 401 when no session is present", async () => {
    const res = await fetch(`${unauthBase}/chat/explain`, {
      method: "POST",
  ...
  it("returns 400 when authenticated request has invalid body", async () => {
    const res = await fetch(`${authedBase}/chat/explain`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
  ...
  it("returns 502 when the model returns invalid/empty JSON", async () => {
    ...
    const bad = await fetch(`${authedBase}/chat/explain`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "hello" }),
    });
    ...
    const empty = await fetch(`${authedBase}/chat/explain`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "hello" }),
    });
    ...
    const emptyMeaning = await fetch(`${authedBase}/chat/explain`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "hello" }),
    });
    expect(emptyMeaning.status).toBe(502);
    const body3 = (await emptyMeaning.json()) as { error?: string };
  ...
  it("returns 429 after exceeding the per-user rate limit", async () => {
    ...
    for (let i = 0; i < max; i++) {
      __testing.resetDailyLimit();
      const ok = await fetch(`${authedBase}/chat/explain`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: "hello" }),
      });
    ...
    const limited = await fetch(`${authedBase}/chat/explain`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "hello" }),
    });
  ...
  it("returns 403 daily_limit_reached after exceeding the daily cap", async () => {
    ...
    __testing.resetRateLimit();
    const blocked = await fetch(`${authedBase}/chat/explain`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "hello" }),
    });
